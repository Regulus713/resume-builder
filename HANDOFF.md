...
- **11 templates**: Classic, Sidebar, Modern, Minimal, Compact, Elegant,
  ...
  gitignored — contains personal info): dark accent header band, gray left
...
- **Data model**: `ResumeData` in `src/types.ts` — personal, summary,
  experience[], education[], skills[], languages[]. `usePersistentState`
  merges stored state over defaults so new fields don't break old saves.

## Architecture notes

Read `AGENTS.md` first — it documents the contentEditable caret-preservation
approach, the `updaters()` pattern, print plumbing, and file map.

## Not implemented (possible next steps)

- Undo/redo, drag-to-reorder in preview
- Custom sections / custom contact fields
